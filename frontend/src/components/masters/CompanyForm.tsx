...
function CompanyFields({ register, errors }: { register: UseFormRegister<CompanyFormData>; errors: FieldErrors<CompanyFormData> }) {
  return (
    <>
...
function buildDefaults(editData?: Company | null): CompanyFormData {
  return editData
    ? {
...
export function CompanyFormBody({ formId, editData, onSubmit }: CompanyFormBodyProps) {
...
export default function CompanyForm({ open, onOpenChange, onSubmit, editData }: CompanyFormProps) {
  ...
  const handle = async (data: CompanyFormData) => {
    await onSubmit(data);
    onOpenChange(false);
  };
