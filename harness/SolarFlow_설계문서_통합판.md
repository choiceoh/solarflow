...
아마란스 웹 업로드 자동화(본격 배포 전 리허설 단계):
- SolarFlow는 아마란스 출고 업로드용 `.xlsx` 파일과 `amaranth_upload_jobs` 작업을 생성한다.
- 별도 Playwright 워커가 로그인 세션을 재사용하고, 필요 시 자동화 전용 PC의 로컬 환경변수로 로그인 fallback을 수행해 `출고등록엑셀업로드` 화면에서 `기능모음 → 엑셀 업로드 → 변환확인`을 실행한다.
- 워커의 SolarFlow API 접근은 사용자 access token 또는 `/api/v1/export/amaranth/*`에만 제한되는 `SOLARFLOW_AMARANTH_RPA_TOKEN`으로 인증한다.
- 자동화 성공 확신이 없거나 화면이 달라지면 `manual_required`로 남기고, 저장된 엑셀 파일과 실패 스크린샷으로 사람이 이어서 처리한다.
...
- 진입: 입고관리 화면에서 PDF/사진 드래그 → 해외직수입 입고등록 창 자동 오픈 → OCR 입력값 확인창 표시
- 반영: B/L 입력/수정 폼의 B/L번호, 실제입항일, 포워더, 항구, 입고품목, 면장 CIF 원화금액, 환율을 자동 채움
- 안전장치: 확인창에서 사용자가 확인하기 전에는 입력칸에 반영하지 않고 DB 저장도 하지 않음
